/* linux/drivers/video/exynos/decon_7880/decon_reg_7880.c
 *
...
void decon_reg_set_dispif_size(u32 id, enum decon_dsi_mode dsi_mode, int dsi_idx, u32 width, u32 height)
{
	u32 val;
...
void decon_reg_configure_lcd(u32 id, enum decon_dsi_mode dsi_mode, struct decon_lcd *lcd_info)
{
	...
	if (lcd_info->dsc_enabled) {
		...
		decon_reg_set_dispif_size(id, dsi_mode, 0, lcd_info->xres, lcd_info->yres);
		if (dsi_mode == DSI_MODE_DUAL_DSI)
			decon_reg_set_dispif_size(id, dsi_mode, 1, lcd_info->xres, lcd_info->yres);
...
void decon_reg_config_dsc_size(u32 id, enum decon_dsi_mode dsi_mode, struct decon_lcd *lcd_info)
{
	...
	decon_reg_set_dispif_size(id, DSI_MODE_SINGLE, 0, width, lcd_info->yres);
}
...
void decon_reg_init_probe(u32 id, u32 dsi_idx, struct decon_param *p)
{
	...
	*/
	...
	if (lcd_info->dsc_enabled) {
		...
		decon_reg_set_dispif_size(id, psr->dsi_mode, 0, lcd_info->xres, lcd_info->yres);
		if (psr->dsi_mode == DSI_MODE_DUAL_DSI)
			decon_reg_set_dispif_size(id, psr->dsi_mode, 1, lcd_info->xres, lcd_info->yres);
...
void decon_reg_set_int(u32 id, struct decon_mode_info *psr, u32 en)
{
	...
	if (en) {
		val = INTERRUPT_INT_EN | INTERRUPT_FIFO_LEVEL_INT_EN | INTERRUPT_DPU0_INT_EN;
		if (psr->psr_mode == DECON_MIPI_COMMAND_MODE) {
			val |= INTERRUPT_FRAME_DONE_INT_EN;
			val |= INTERRUPT_DISPIF_VSTATUS_INT_EN | INTERRUPT_DISPIF_VSTATUS_VSA;
		}
...
const signed long decon_clocks_table[][CLK_ID_MAX] = {
	...
	{   252, 252,   400,    66,       252, 1440 * 2560,     MIC_COMP_BYPASS,         0},
	{   142, 252,   400,    66,       142, 1080 * 1920,     MIC_COMP_BYPASS,         0},
};
